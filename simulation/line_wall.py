...
class LineWall:
    ...
    def check_circle_intercept(self, circle_start, circle_end, radius):
        ...
        su = s/(distance(self.end, self.start))
        seg_end_ext = self.end + su * p
        seg_start_ext = self.start - su * p
        ...
        pC = closest_point_to_seg(self.start, self.end, p2)

        if (not point_on_line(self.start, self.end, pC)) or (plc_dist < radius):
            # The point is not on the line, this is a pole or shallow angle scenario
            # Get the point closest to pC
            if distance(self.start, pC) <= distance(self.end, pC):
                # self.start is closest
